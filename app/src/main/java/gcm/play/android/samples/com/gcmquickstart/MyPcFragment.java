 ...
 */
public class MyPcFragment extends Fragment {
    ...
    final static String TAG = "MyPcFragemt";
    RecyclerView recyclerView;
    RecyclerView.LayoutManager layoutManager = null;
    String[][] forecastArray = {{" "}, {" "}};

    public MyPcFragment(Context c) {
        context = c;
    }
    ...
    public View onCreateView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        final View rootView = inflater.inflate(R.layout.activity_main_recycler, container,
                false);
        ...
        if (forecastArray != null) {
           // forecastArray[1] = refineTime(forecastArray[1]);
            ...
            adapter = new CustomAdapter(context, getData());
            ...
            recyclerView.setOnScrollListener(new HidingScrollListener() {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    ...
                    RecyclerView.ViewHolder v =  recyclerView.getChildViewHolder(recyclerView.getChildAt(0));


                   TextView t1 = (TextView)v.itemView.findViewById(R.id.tvDateCard);
                    MyDB enter = new MyDB(recyclerView.getContext());

                    View layout = inflater.inflate(R.layout.toast_layout,(ViewGroup)rootView.findViewById(R.id.toast_layout_root));


                    TextView text = (TextView) layout.findViewById(R.id.text);
                    text.setText(t1.getText().toString());

                    final Toast t = new Toast(context);
                    t.setDuration(Toast.LENGTH_SHORT);
                    t.setView(layout);
                    t.setGravity(0,0,-320);
                    t.setView(layout);
                    t.show();
    ...
    private String[] refineTime(String[] timeString) {
        ...
        for (int i = 0; i < timeString.length; i++) {
            Log.v(TAG,"timestring is"+timeString[i]);
            int length = timeString[i].length();
            int day = 0;
 ...
 //           int month = Integer.parseInt(timeString[i].substring(5, 7));
            ...
            for(int k = 5; k<timeString[i].length();k++){

                if(timeString[i].charAt(k)=='-'){
                    day = Integer.parseInt(timeString[i].substring(k,k+2));
                }
            }
            ...
            int hour = Integer.parseInt(timeString[i].substring(length-5, length-2));
            if (hour > 12) {
